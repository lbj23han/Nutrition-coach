// 네이티브 전용 (iOS / Android)
// Metro가 .native.tsx 파일을 자동으로 선택합니다.
...
export function FoodCamera({ visible, onClose, onResult }: FoodCameraProps) {
  ...
  const handleGallery = async () => {
    ...
    setIsAnalyzing(true);
    try {
      const base64 = await compressAndEncode(picked.assets[0].uri);
...
const styles = StyleSheet.create({
  ...
  frameHint: { color: 'rgba(255,255,255,0.8)', fontSize: 13, textAlign: 'center' },
  bottomBar: { alignItems: 'center', paddingBottom: 48, height: 140, justifyContent: 'center' },
  captureButton: {
    width: 80, height: 80, borderRadius: 40,
    backgroundColor: 'rgba(255,255,255,0.3)', justifyContent: 'center', alignItems: 'center',
    borderWidth: 3, borderColor: COLORS.white,
  },
  captureButtonInner: { width: 62, height: 62, borderRadius: 31, backgroundColor: COLORS.white },
  analyzingContainer: { alignItems: 'center', gap: 12 },
  analyzingText: { color: COLORS.white, fontSize: 15, fontWeight: '500' },
  permissionContainer: {
    flex: 1, justifyContent: 'center', alignItems: 'center',
    padding: 24, backgroundColor: COLORS.background, gap: 16,
  },
